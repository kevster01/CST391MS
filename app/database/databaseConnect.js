...
var _bookDAO = require("../models/userDAO");
...
var dbConnector = /*#__PURE__*/function () {
  function dbConnector(host, port, username, password) {
    ...
    (0, _defineProperty2["default"])(this, "schema", "cst391milestone");
  ...
  (0, _createClass2["default"])(dbConnector, [{
      ...
      this.pool.getConnection(function (err, connection) {
        ...
        connection.query('SELECT * FROM users', function (err, rows, fields) {
          ...
          for (var x = 0; x < rows.length; ++x) {
            users.push(new _userDAO.userDAO(rows[x].Name, rows[x].Email, rows[x].Password));
          }
          callback(users);
        });
      });
    ...
    key: "findUsersByName",
    value: function findUsersByName(name, callback) {
      var users = [];
      this.pool.getConnection(function (err, connection) {
        if (err) throw err;
        connection.query("SELECT * FROM users WHERE NAME LIKE '%" + name + "%'", function (err, rows, fields) {
          connection.release();
          if (err) throw err;
          for (var x = 0; x < rows.length; ++x) {
            users.push(new _userDAO.userDAO(rows[x].Name, rows[x].Email, rows[x].Password));
    ...
    key: "findUserssByEmail",
    value: function findUsersByEmail(email, callback) {
      var users = [];
      this.pool.getConnection(function (err, connection) {
        if (err) throw err;
        connection.query("SELECT * FROM users WHERE GENRE LIKE '%" + users + "%'", function (err, rows, fields) {
          connection.release();
          if (err) throw err;
          for (var x = 0; x < rows.length; ++x) {
            users.push(new _userDAO.userDAO(rows[x].Name, rows[x].Email, rows[x].Password));
    ...
    key: "createUser",
    value: function createUser(b, callback) {
            ...
            while (1) {
              switch (_context.prev = _context.next) {
                ...
                case 3:
                  ...
                  _context.next = 6;
                  return connection.query('INSERT INTO users (name, email, password) VALUES(?,?,?)', [b.Name, b.Email, b.Password]);
                case 6:
    ...
    key: "updateUser",
    value: function updateUser(b, callback) {
      this.pool.getConnection( /*#__PURE__*/function () {
            ...
            while (1) {
              switch (_context2.prev = _context2.next) {
                ...
                case 3:
                  ...
                  connection.query = util.promisify(connection.query);
                  _context2.next = 7;
                  return connection.query('UPDATE userss SET name=?, email=?,  password=?', [b.Name, b.Email, b.Password]);
                case 7:
    ...
    key: "deleteUserById",
    value: function deleteUserById(id, callback) {
      this.pool.getConnection( /*#__PURE__*/function () {
